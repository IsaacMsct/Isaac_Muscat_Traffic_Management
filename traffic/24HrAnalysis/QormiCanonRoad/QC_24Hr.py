...
def print_summary_table(data):
    print(f"{'Time Period':<15}{'Direction Attard':<20}{'Direction Hamrun':<20}")
    for block, situations in sorted(data.items()):
        attard = situations['Attard']
        hamrun = situations['Hamrun']
        print(f"{block:<15}{attard:<20}{hamrun:<20}")

if __name__ == '__main__':
    log_file_path = 'D:\\school\\TrafficDetection\\venv\\qormicanonroad_traffic_report.txt'
