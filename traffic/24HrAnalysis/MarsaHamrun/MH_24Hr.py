...
def print_summary_table(data):
    print(f"{'Time Period':<15}{'Direction Marsa':<20}{'Direction Santa Venera':<25}{'Direction Qormi':<20}")
    for block, situations in sorted(data.items()):
        marsa = situations['Marsa']
        santa_venera = situations['Santa Venera']
        qormi = situations['Qormi']
        print(f"{block:<15}{marsa:<20}{santa_venera:<25}{qormi:<20}")

if __name__ == '__main__':
    log_file_path = 'D:\\school\\TrafficDetection\\venv\\marsahamrun_traffic_report.txt'
